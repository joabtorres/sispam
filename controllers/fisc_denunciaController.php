...
class fisc_denunciaController extends fiscController {
    ...
    public function denuncia($id) {
        if ($this->checkUser() == 10 || ($this->checkSetor() == 4 || $this->checkSetor() == 10)) {
            ...
            if (!empty($resultado)) {
                ...
                $dados['defesaAdministrativa'] = $crudModel->read('SELECT fd.*, p.numero_protocolo FROM fisc_defesa_denuncia AS fd INNER JOIN protocolo AS p where fd.protocolo_id=p.id and fd.denuncia_id=:id', array('id' => $resultado['id']));

                //salva vistoria
                if (isset($_POST['nSalvaDefesa'])) {
                    $cadForm = array();
                    $cadForm['data'] = $this->formatDateBD($_POST['nData']);
                    $cadForm['descricao'] = addslashes($_POST['nDescricao']);
                    $cadForm['protocolo_id'] = addslashes($_POST['nVincula']);
                    $cadForm['denuncia_id'] = $resultado['id'];
                    if ($crudModel->create("INSERT INTO fisc_defesa_denuncia (denuncia_id, protocolo_id, descricao, data) VALUES (:denuncia_id, :protocolo_id, :descricao, :data)", $cadForm)) {
    ...
    public function excluirdefesa($id) {
        if ($this->checkUser() == 10 || ($this->checkSetor() == 4 || $this->checkSetor() == 10)) {
            $crudModel = new crud_db();
            $resultado = $crudModel->read_specific("SELECT * FROM fisc_defesa_denuncia WHERE md5(id)=:id", array('id' => addslashes($id)));
            if ($crudModel->remove("DELETE FROM fisc_defesa_denuncia WHERE md5(id)=:cod", array('cod' => addslashes($id)))) {
                $url = "location: " . BASE_URL . 'fisc_denuncia/denuncia/' . md5($resultado['denuncia_id']);
                header($url);
            }
        } else {
