...
class fiscController extends controller {
    ...
    public function get_protocolo() {
        if (isset($_POST) && is_array($_POST) && !empty($_POST)) {
            ...
            $resultado = $crudModel->read("SELECT p.*, po.objetivo FROM protocolo as p INNER JOIN  protocolo_objetivo as po where po.id=p.objetivo_id");

            if (!isset($protocolo_id)) {
                echo '<option value="" selected = "selected" disabled="disabled">Selecione um protocolo </option>';
            }
            foreach ($resultado as $indice) {
                if (isset($protocolo_id) && $indice['id'] == $protocolo_id) {
